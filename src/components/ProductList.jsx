  ...
  return (
    <div style={{ paddingBottom: '40px' }}>
      ...
      {completedItems.length > 0 && (
        <>
          <h3 style={{ margin: '20px 0 10px', color: 'var(--text-secondary)', fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '1px' }}>
            Completed
          </h3>
...
const Item = ({ item, editingId, editForm, setEditForm, handleToggle, handleDelete, startEdit, cancelEdit, saveEdit, isCompleted }) => {
  ...
  if (isCompleted) {
    return (
      <motion.li layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ marginBottom: 'var(--spacing-sm)' }}>
        {content}
      </motion.li>
    );
  }
